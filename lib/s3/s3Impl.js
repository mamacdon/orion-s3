...
//	var utf8encode = Crypto.charenc.UTF8.stringToBytes;
	...
	/**
	 * Stores the user's salted passphrase so they don't have to provide it on every request. Not persisted.
	 */
	var sessionKey;
	 ...
	 * is not persisted, so must be re-entered whenever the window is reopened.
	 */
...
//		}
		...
		if (!sessionKey) {
			var salt = getItem(localStorage, 'salt'), pass;
			if (salt) {
				pass = promptFor('Enter your passphrase:');
				assert(pass, 'Passphrase not provided.');
				sessionKey = salt + pass;
			} else {
				pass = promptFor('Choose a passphrase (6 or more characters):');
				assert(pass, 'Passphrase not provided.');
				assert(pass.length > 5, 'Passphrase too short.');
				salt = Crypto.util.bytesToBase64(Crypto.util.randomBytes(32));
				sessionKey = salt + pass;
			}
			setItem(localStorage, 'salt', salt);
		}
		try {
			var access = getCryptedItem(localStorage, 'accessKey', sessionKey) || promptFor('Enter your "Access Key Id":\n(will be encrypted and saved in localStorage)');
			assert(access, 'Access Key Id not provided.');
			setItem(localStorage, 'accessKey', encrypt(access, sessionKey));

			var secret = getCryptedItem(localStorage, 'secretKey', sessionKey) || promptFor('Enter your "Secret Access Key":\n(will be encrypted and saved in localStorage)');
			assert(secret, 'Secret Access Key not provided.');
			setItem(localStorage, 'secretKey', encrypt(secret, sessionKey));
