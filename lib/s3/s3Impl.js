 ...
 ******************************************************************************/
/*global Crypto define DOMParser sessionStorage prompt XMLSerializer*/
...
//	var utf8encode = Crypto.charenc.UTF8.stringToBytes;
	...
	function promptForKeys() {
		var aKey = sessionStorage.getItem('accessKey'), sKey = sessionStorage.getItem('secretKey');
		if (!aKey) {
			aKey = prompt('Enter your Access Key Id\n(saved in sessionStorage):');
			aKey = aKey && aKey.trim();
		}
		if (!aKey) {
			throw 'Access Key Id was not provided.';
		}
		sessionStorage.setItem('accessKey', aKey);

		if (!sKey) {
			sKey = prompt('Enter your Secret Access Key\n(saved in sessionStorage):');
			sKey = sKey && sKey.trim();
		}
		if (!sKey) {
			throw 'Secret Access Key was not provided.';
		}
		sessionStorage.setItem('secretKey', sKey);
		return [aKey, sKey];
	}
