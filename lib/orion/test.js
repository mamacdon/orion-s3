...
define(['dojo/_base/Deferred', 'dojo/_base/lang', 'orion/assert'], function(Deferred, lang, assert) {
				...
				try {
					...
					var serviceProvider = provider.registerServiceProvider("orion.test.runner", {
						run: function() {
							...
							Deferred.when(_run(testName), lang.hitch(result, "resolve"));
							return result;
						},
						list: function() {
							return _list("", obj);
					...
					provider.connect(function() {
						...
						Deferred.when(_run(optTestName), lang.hitch(result, "resolve"));
					});
					return result;
				} catch (e) {
					// fall through
